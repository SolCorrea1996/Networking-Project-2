...
class Pkt:
    def __init__(self, seqnum, acknum, checksum, payload):
        ...
        self.payload = payload[:Msg.MSG_SIZE]
...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        self.next_seqnum = 0  #ALTERNANITY BIT (0 or 1): self.next_seqnum = random.randint(0, self.seqnum_limit-1)
        self.packet_in_transit = None #Store a Package being sent
    ...
    def output(self, message):
        if self.packet_in_transit is None:
            #Create a packet with the message data to layer 5
            payload = message.data
            packet = Pkt(self.next_seqnum, acknum=0, checksum=0, payload=payload)
            pkt_insert_checksum(packet) #Insert and compute the checksum to the packet
            to_layer3(self, packet) #Send the packet to layer 3
            self.packet_in_transit = packet #Store the packet in transit
            
            if TRACE > 0:
                print(f"EntityA: Timer running={self.timer_running}. Attempting to start timer." )
            ...
            if not self.timer_running:
                ...
                if TRACE > 0:
                    print(f"EntityA: Timer started." )
            else:
                if TRACE > 0:
                    print(f"EntityA: Timer already running. Skipping timer start." )
                
            # Print the packet if TRACE > 0 , debugging messages
            if TRACE > 0:
                print(f"EntityA: Sending packet with seqnum={self.next_seqnum}, payload={packet.payload}, payload size={len(packet.payload)}, checksum={packet.checksum}" )
    ...
    def input(self, packet):
        if not pkt_is_corrupt(packet) and packet.acknum == self.next_seqnum:
            # Receive a valid packet
            self.packet_in_transit = None #Clear the packet in transit
            self.retransmission_count = 0 #Reset the retransmission counter
            ...
            if self.timer_running:
                ...
                self.timer_running = False
                if TRACE > 0:
                    print(f"EntityA: Timer stopped.")
            else:
                if TRACE > 0:
                    print(f"EntityA: Timer not running. Skipping timer stop.")
                
            #ALTERNATING BIT (0 or 1): self.next_seqnum = random.randint(0, self.seqnum_limit-1)
            self.next_seqnum = 1 - self.next_seqnum
            
            # Print the packet if TRACE > 0 , for debugging messages
            if TRACE > 0:
                print(f"EntityA: Received valid ACK for seqnum={packet.acknum}")

        else:
            #If the packet is corrupted or the acknum does not match the expected seqnum
            if TRACE > 0:
                print(f"EntityA: Received corrupted or unexpected ACK with acknum={packet.acknum}")
            

    # Called when A's timer goes off.
    def timer_interrupt(self):
        if self.packet_in_transit is not None and self.retransmission_count < self.max_retransmissions:
            self.retransmission_count += 1
            
            #Restart the timer only if its not already running
            if self.timer_running:
                stop_timer(self)
                self.timer_running = False
                if TRACE > 0:
                    print(f"EntityA: Timer stopped due to retransmission attempt.")
            
            #Resend the packet if the timer expires
            to_layer3(self, self.packet_in_transit)
            ...
            # Debugging message
            if TRACE > 0:
                print(f"EntityA: Timer expired, retransmitting packet with seqnum={self.packet_in_transit.seqnum}, checksum={self.packet_in_transit.checksum}")
            ...
            if self.timer_running:
                ...
                self.timer_running = False
                if TRACE > 0:
                    print(f"EntityA: Timer stopped due to retransmission limit reached.")
            
            print(f"EntityA: Failed to deliver the packet after {self.max_retransmissions} attempts. Dropping packet or logical critical error")
...
class EntityB:
    ...
    def __init__(self, seqnum_limit):
        self.seqnum_limit = seqnum_limit
        self.expected_seqnum = 0 #Initialize waiting for packet with seqnum 0
        self.last_ack_packet = None #last ack packet sent

    # Called from layer 3, when a packet arrives for layer 4 at EntityB.
    # The argument `packet` is a Pkt containing the newly arrived packet.
    def input(self, packet):
        # Verify if the packet is not corrupt and has the expected sequence number
        if not pkt_is_corrupt(packet) and packet.seqnum == self.expected_seqnum:
            # Ensure the payload has the correct format before sending it to layer 5
            payload = packet.payload[:Msg.MSG_SIZE]

            # Send the payload to layer 5 and ensure the payload has the correct format
            if isinstance(payload, bytes) and len(payload) == Msg.MSG_SIZE:
                to_layer5(self, Msg(payload))
            else:
                print("ERROR: Payload is incorrectly formatted before sending to layer 5.")
        
            # Create and send an ACK packet
            ack_packet = Pkt(seqnum=0, acknum=self.expected_seqnum, checksum=0, payload='ackackackackackackac'.encode())
            pkt_insert_checksum(ack_packet)
            to_layer3(self, ack_packet)
            self.last_ack_packet = ack_packet
        
            # Alternate the expected sequence number between 0 and 1
            self.expected_seqnum = 1 - self.expected_seqnum

            # Debugging message
            if TRACE > 0:
                print(f"EntityB: Received packet with seqnum={packet.seqnum}, sent ACK={ack_packet.acknum}, checksum={ack_packet.checksum}")

        else:
            # If the packet is corrupted or the acknum does not match the expected seqnum, resend the last ACK
            if self.last_ack_packet is not None:
                # Send an ACK with the last packet's sequence number
                nack_packet = Pkt(seqnum=0, acknum=self.last_ack_packet.acknum, checksum=0, payload= b'nacknacknacknacknack')
                pkt_insert_checksum(nack_packet) #Compute the checksum N-ACK
                #Send the N-ACK to layer 3
                to_layer3(self, nack_packet)
            
                #Debugging message
                if TRACE > 0:
                    print(f"EntityB: Received out-of-order or corrupted packet. Sending N-ACK with acknum={nack_packet.acknum}")
            else:
                #Debugging message
                if TRACE > 0:
                    print(f"EntityB: No previous ACK to resend for corrupted or out-of-order packet.")
    # Called when B's timer goes off.
    def timer_interrupt(self):
        # EntityB does not have a timer
        pass
